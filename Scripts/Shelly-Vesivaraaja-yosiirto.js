...
// KOODI
let url = "https://api.spot-hinta.fi/JustNowRanksAndPrice?priorityHours=22,23,0,1,2,3,4,5,6&priceModifier=" + Hintaero + "&ranksAllowed=" + Rankit.join();
    ...
    Shelly.call("HTTP.GET", { url: url, timeout: 15, ssl_ca: "*" }, function (res, err) {
        ...
        if (hour === -1) {
            if (Varatunnit.indexOf(new Date().getHours()) > -1) {
                ...
                print("WaterBoiler: Virhetilanne. Kuluva tunti on varatunti: rele kytketään tämän tunnin ajaksi.");
            } else {
